...
###########   TRAINING DATASET 1: scTab--10pct-TRAIN ############
...
#remove cells with tissue_general == "bone marrow"
#sctab_train_10pct = sctab_train_10pct[sctab_train_10pct.obs['tissue_general'] != "bone marrow"].copy()
#make sure obs names are unique
sctab_train_10pct.obs_names_make_unique()

#for the scTab-All analysis, remove cells that will be in downstream bone marrow and brain cell atlas evals
...
#what is the overlap in obs indices between the two datasets?
overlap_bonemarrow = np.intersect1d(sctab_train_10pct.obs.index, sctab_train_100pct_bonemarrow_val.obs.index)
print("overlap: ", overlap_bonemarrow.shape[0])

#reomve cells from 10pct dataset that are in the 100pct dataset
sctab_train_10pct = sctab_train_10pct[~sctab_train_10pct.obs.index.isin(overlap_bonemarrow)]

#remove cells from 10pct with donor_id == H18.30.002, to remove overlapping brain cell atlas
sctab_train_10pct = sctab_train_10pct[sctab_train_10pct.obs['donor_id'] != "H18.30.002"]

#QC on sctab data
#get total counts
#QC on hematological dataset
#get the cell-wise total read count
sctab_train_10pct.obs['total_counts'] = sctab_train_10pct.X.sum(axis=1)

#get the expression of mitochondrial genes and express as proportion of total counts
mito_genes = sctab_train_10pct.var_names.str.startswith('MT-')
#print the mito gene names
print(sctab_train_10pct.var_names[mito_genes])
sctab_train_10pct.obs['percent_mito'] = np.sum(sctab_train_10pct[:, mito_genes].X, axis=1) / np.sum(sctab_train_10pct.X, axis=1)

#how many cells have percent mito less than 0.1 and total counts greater than 1000?
print(sctab_train_10pct.obs[(sctab_train_10pct.obs['percent_mito'] < 0.1) & (sctab_train_10pct.obs['total_counts'] > 1000)].shape[0])
print(sctab_train_10pct.obs.shape[0])

#make a QC_pass variable based on these cutoffs
sctab_train_10pct.obs['QC_pass'] = (sctab_train_10pct.obs['percent_mito'] < 0.1) & (sctab_train_10pct.obs['total_counts'] > 1000)
...
plt.scatter(sctab_train_10pct.obs['percent_mito'],
            sctab_train_10pct.obs['total_counts'],
            c = sctab_train_10pct.obs['QC_pass'], 
            alpha=0.005,
            cmap = QC_color_map)
...
sctab_train_10pct.var_names = sctab_train_10pct.var['feature_id']
sctab_train_10pct.var.index = sctab_train_10pct.var['feature_id']

#filter the data
sctab_train_10pct = sctab_train_10pct[sctab_train_10pct.obs['QC_pass'], :].copy()
...
subset1, subset2 = train_test_split(sctab_train_10pct.obs.index, test_size=0.5, random_state=seed)
...
#subset the data
sctab_train_10pct_subset1 = sctab_train_10pct[subset1, :].copy()
sctab_train_10pct_subset2 = sctab_train_10pct[subset2, :].copy()


print(sctab_train_10pct_subset1)
print(sctab_train_10pct_subset2)
...
gene_name_to_id = {}
for i in range(sctab_train_10pct.var.shape[0]):
    gene_name_to_id[sctab_train_10pct.var['feature_name'][i]] = sctab_train_10pct.var['feature_id'][i]
...
sctab_train_100pct_bonemarrow.var_names = sctab_train_10pct.var['feature_id'].astype(str)
...
mito_genes = sctab_train_10pct.var['feature_name'].str.startswith('MT-')
#print the mito gene names
...
sctab_train_10pct_blood_gene_ids = sctab_train_10pct.var['feature_id']
...
sctab_train_10pct_subset1 = sctab_train_10pct_subset1[:, common_gene_ids].copy()
sctab_train_10pct_subset2 = sctab_train_10pct_subset2[:, common_gene_ids].copy()
...
#BaseModel: Only sctab_train_10pct_subset1
BaseModel_scTabAll = sctab_train_10pct_subset1.copy()
#reindex obs with a new index
BaseModel_scTabAll.obs.index = range(BaseModel_scTabAll.obs.shape[0])

#Model1: sctab_train_10pct_subset1 + sctab_train_10pct_blood_subset2
Model1_AddscTabAll = ad.concat([sctab_train_10pct_subset1, sctab_train_10pct_subset2],
                                 label = 'dataset',
                                 keys = ['scTabAll_subset1','scTabAll_subset2'],
                                 index_unique = "_")
#reindex obs with a new index
Model1_AddscTabAll.obs.index = range(Model1_AddscTabAll.obs.shape[0])

#Model 2: sctab_train_10pct_subset1 + hematologic_cca
Model2_AddCCAHematologic = ad.concat([sctab_train_10pct_subset1, hematologic_cca],
                                 label = 'dataset',
                                 keys = ['scTabAll_subset1','CCAHematologic'],
                                 index_unique = "_")
#reindex obs with a new index
Model2_AddCCAHematologic.obs.index = range(Model2_AddCCAHematologic.obs.shape[0])

#Model 3: sctab_train_10pct_subset1 + weissman_concat
Model3_AddPerturbSeq = ad.concat([sctab_train_10pct_subset1, weissman_concat],
                                 label = 'dataset',
                                 keys = ['scTabAll_subset1','WeissmanPerturbSeq'],
                                 index_unique = "_")
#reindex obs with a new index
Model3_AddPerturbSeq.obs.index = range(Model3_AddPerturbSeq.obs.shape[0])

#Model 4: sctab_train_10pct_subset1 + sctab_train_100pct_bonemarrow
Model4_AddscTabBoneMarrow = ad.concat([sctab_train_10pct_subset1, sctab_train_100pct_bonemarrow],
                                 label = 'dataset',
                                 keys = ['scTabAll_subset1','scTabBoneMarrow'],
                                 index_unique = "_")
#reindex obs with a new index
Model4_AddscTabBoneMarrow.obs.index = range(Model4_AddscTabBoneMarrow.obs.shape[0])

#Model 5: sctab_train_10pct_subset1 + joung_data_subsample
Model5_AddJoungData = ad.concat([sctab_train_10pct_subset1, joung_data_subsample],
                                 label = 'dataset',
                                 keys = ['scTabAll_subset1','JoungTF'],
                                 index_unique = "_")
#reindex obs with a new index
...
BaseModel_scTabAll.X = sp.sparse.csr_matrix(BaseModel_scTabAll.X)
Model1_AddscTabAll.X = sp.sparse.csr_matrix(Model1_AddscTabAll.X)
...
BaseModel_scTabAll.write(processed_data_path + 'BaseModel_scTabAll_seed' + str(seed) + '_allbase_TrainingData.h5ad')
Model1_AddscTabAll.write(processed_data_path + 'Model1_AddscTabAll_seed' + str(seed) + '_allbase_TrainingData.h5ad')
Model2_AddCCAHematologic.write(processed_data_path + 'Model2_AddCCAHematologic_seed' + str(seed) + '_allbase_TrainingData.h5ad')
Model3_AddPerturbSeq.write(processed_data_path + 'Model3_AddPerturbSeq_seed' + str(seed) + '_allbase_TrainingData.h5ad')
Model4_AddscTabBoneMarrow.write(processed_data_path + 'Model4_AddscTabBoneMarrow_seed' + str(seed) + '_allbase_TrainingData.h5ad')
Model5_AddJoungData.write_h5ad(processed_data_path + 'Model5_AddJoungTFAtlas_seed' + str(seed) + '_allbase_TrainingData.h5ad')

print(BaseModel_scTabAll)
print(BaseModel_scTabAll)
print(Model2_AddCCAHematologic)
print(Model3_AddPerturbSeq)
print(Model4_AddscTabBoneMarrow)
print(Model5_AddJoungData)
...
Model6_TrainingData.write(processed_data_path + 'Model6_CCAHematologic_seed' + str(seed) + '_allbase_TrainingData.h5ad')
...
Model7_TrainingData.write(processed_data_path + 'Model7_PerturbSeq_seed' + str(seed) + '_allbase_TrainingData.h5ad')
...
Model8_TrainingData.write(processed_data_path + 'Model8_scTabBoneMarrow_seed' + str(seed) + '_allbase_TrainingData.h5ad')
...
Model9_TrainingData.write(processed_data_path + 'Model9_JoungTFAtlas_seed' + str(seed) + '_allbase_TrainingData.h5ad')
